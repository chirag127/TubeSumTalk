...
function startSpeaking() {
    ...
    summaryContent.innerHTML = words
        .map(
            (word, index) =>
...
function displaySummary(result) {
    ...
    // Update video information if not already set
    if (videoTitle.textContent === "Loading video information...") {
        // Try to get the title from the parent page again
        try {
            const parentTitle = window.parent.document.title;
            if (parentTitle) {
                // YouTube titles typically end with " - YouTube"
            ...
            // If we can't access the parent title, use a generic title
    ...
    } else if (result.transcript_available === false) {
        summaryContent.innerHTML =
            "<p>No subtitles available for this video. Unable to generate summary.</p>";
        readAloudButton.disabled = true;
    } else {
