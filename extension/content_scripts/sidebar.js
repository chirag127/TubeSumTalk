...
class TubeSumTalkSidebar {
  ...
  async init() {
    ...
    const voiceSelect = this.sidebarElement.querySelector('#tubesumtalk-voice-select');
    if (voiceSelect) {
  ...
  getSidebarHTML() {
    return `
      <div class="tubesumtalk-header">
        <h3>
          <img src="${chrome.runtime.getURL('icons/icon48.png')}" alt="TubeSumTalk">
          TubeSumTalk
        </h3>
  ...
  setupEventListeners() {
    // Toggle sidebar
    const toggleButton = this.sidebarElement.querySelector('#tubesumtalk-toggle');
    if (toggleButton) {
      toggleButton.addEventListener('click', () => this.toggleSidebar());
    }
    
    // Play/pause TTS
    const playPauseButton = this.sidebarElement.querySelector('#tubesumtalk-play-pause');
    if (playPauseButton) {
      playPauseButton.addEventListener('click', () => this.togglePlayPause());
    }
    
    // Speed slider
    const speedSlider = this.sidebarElement.querySelector('#tubesumtalk-speed-slider');
    const speedValue = this.sidebarElement.querySelector('#tubesumtalk-speed-value');
    ...
    // Settings button
    const settingsButton = this.sidebarElement.querySelector('#tubesumtalk-settings');
    if (settingsButton) {
      settingsButton.addEventListener('click', () => this.showSettingsPopup());
    }
    
    // Resize handle
    const resizeHandle = this.sidebarElement.querySelector('#tubesumtalk-resize-handle');
    if (resizeHandle) {
  ...
  showSettingsPopup() {
    ...
    popup.innerHTML = `
      <div class="tubesumtalk-settings-content">
        ...
        <div class="tubesumtalk-setting">
          <label for="tubesumtalk-settings-speed">Speed:</label>
          <input type="range" id="tubesumtalk-settings-speed" class="tubesumtalk-speed-slider" 
                min="0.5" max="2" step="0.1" value="${this.ttsSettings.rate}">
          <span id="tubesumtalk-settings-speed-value">${this.ttsSettings.rate.toFixed(1)}×</span>
        </div>
        ...
        <div class="tubesumtalk-setting">
          ...
          <span id="tubesumtalk-settings-pitch-value">${this.ttsSettings.pitch.toFixed(1)}</span>
        </div>
    ...
    const speedValue = popup.querySelector('#tubesumtalk-settings-speed-value');
    speedSlider.addEventListener('input', () => {
      const rate = parseFloat(speedSlider.value);
      speedValue.textContent = `${rate.toFixed(1)}×`;
    });
    ...
    const pitchValue = popup.querySelector('#tubesumtalk-settings-pitch-value');
    ...
    saveButton.addEventListener('click', () => {
      ...
      // Update UI
      const mainVoiceSelect = this.sidebarElement.querySelector('#tubesumtalk-voice-select');
      const mainSpeedSlider = this.sidebarElement.querySelector('#tubesumtalk-speed-slider');
      const mainSpeedValue = this.sidebarElement.querySelector('#tubesumtalk-speed-value');
    ...
    // Cancel button
    const cancelButton = popup.querySelector('#tubesumtalk-settings-cancel');
  ...
  setupResizeHandling(resizeHandle) {
    ...
    const startResize = (e) => {
      startX = e.clientX;
      startWidth = parseInt(getComputedStyle(this.sidebarElement).width, 10);
    ...
    const stopResize = () => {
      ...
      const width = parseInt(getComputedStyle(this.sidebarElement).width, 10);
  ...
  async loadTTSSettings() {
    return new Promise((resolve) => {
      chrome.storage.sync.get(['ttsVoice', 'ttsRate', 'ttsPitch', 'sidebarWidth'], (result) => {
        if (result.ttsVoice) this.ttsSettings.voice = result.ttsVoice;
        if (result.ttsRate) this.ttsSettings.rate = result.ttsRate;
        if (result.ttsPitch) this.ttsSettings.pitch = result.ttsPitch;
  ...
  setVideoDetails(details) {
    ...
    const titleElement = this.sidebarElement.querySelector('#tubesumtalk-video-title');
  ...
  setSummary(summary) {
    ...
    const summaryElement = this.sidebarElement.querySelector('#tubesumtalk-summary');
    if (summaryElement) {
